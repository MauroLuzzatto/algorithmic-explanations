...
class ShapExplanation(ExplanationBase):
    ...
    def __init__(
        ...
        super(ShapExplanation, self).__init__(
              config
        )
        """
        ...
        natural_language_text_empty = "The {} features which were most important for this particular sample were (from highest to lowest): {}."
        method_text_empty = "The feature importance was calculated using the SHAP method."
    ...
    def _calculate_importance(self) -> None:
        ...
        assert isinstance(self.explainer.expected_value, float), 'self.explainer.expected_value has wrong type'
    ...
    def get_feature_values(self, sample_index: int = 0):
        ...
        for index in indexes.tolist()[::-1][: self.number_of_features]:
            feature_values.append(
                (self.feature_names[index], self.shap_values[sample_index, index])
            )
    ...
    def get_score(self, sample_index: int = 0) -> float:
        ...
        """
        return (
            np.sum(self.shap_values[sample_index, :]) + self.explainer.expected_value
        )
    ...
    def shap_plot(self, sample_index: int = 0) -> None:
        ...
        shap.force_plot(
            ...
            shap_values=np.around(self.shap_values[sample_index, :], decimals=2),
    ...
    def log_output(self, sample_index: int) -> None:
        ...
        self.logger.debug("The y_value was: {}".format(self.y.values[sample_index][0]))
